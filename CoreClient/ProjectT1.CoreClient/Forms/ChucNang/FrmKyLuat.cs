...
namespace ProjectT1.CoreClient {
    public partial class FrmKyLuat : XtraForm {
        ...
        private BindingList<KyLuatDTO> _bindingListMain = new();

        #region Contructor & FormLoad
        public FrmKyLuat() {
            InitializeComponent();
            IBusObj = new CNKyLuatClient(_httpClient);
        ...
        private void CommonConfig() {
            ...
            fThoiGianXayRa.SetDefaultDateEditSettings();
            fNgayBatDauThiHanh.SetDefaultDateEditSettings();
            fNgayHetHieuLuc.SetDefaultDateEditSettings();
        ...
        private async Task LoadDataMainGrid() {
            try {
                ...
                _bindingListMain = new BindingList<KyLuatDTO>(res.Result.OrderBy(x => x.MaSo).ToList());
        ...
        private async void btnSubmit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            try {
                ...
                var obj = new KyLuatDTO {
                    ...
                    ThoiGianXayRa = fThoiGianXayRa.EditValue.ConvertToNullableDateTimeDMY(),
                    DiaDiemXayRa = fDiaDiemXayRa.EditValue.SafeToString(),
                    MoTaSuViec = fMoTaSuViec.EditValue.SafeToString(),
                    NhungNguoiChungKien = fNhungNguoiChungKien.EditValue.SafeToString(),
                    IsViPhamChinhSachCongTy = fIsViPhamChinhSachCongTy.EditValue.ConvertToNullableBool(),
                    DienGiai = fDienGiai.EditValue.SafeToString(),
                    HinhThucKyLuat = fHinhThucKyLuat.EditValue.SafeToString(),
                    NgayBatDauThiHanh = fNgayBatDauThiHanh.EditValue.ConvertToNullableDateTimeDMY(),
                    NgayHetHieuLuc = fNgayHetHieuLuc.EditValue.ConvertToNullableDateTimeDMY(),
                };

                // Validate
                clsCommon.CommonHandler.ClearEditControlErrorText(layoutControlGroupEditControl);
                var validator = new KyLuatValidator();
                ...
                if (_mainStatus == MainStatusForm.CREATE) {
                    List<KyLuatDTO> data = new List<KyLuatDTO>();
                    data.Add(obj);
                    ...
                    var objToEdit = (KyLuatDTO)gridViewMain.GetRow(gridViewMain.FocusedRowHandle);
        ...
        private async void btnCancel_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            if (gridViewMain.RowCount > 0) {
                var objMain = (KyLuatDTO)gridViewMain.GetFocusedRow();
