...
namespace ProjectT1.CoreClient {
    public partial class FrmDMTrinhDoHocVan : XtraForm {
        HttpClient _httpClient = new();
        DMTrinhDoHocVanClient IBusObj;

        private MainStatusForm _mainStatus;
        private Guid _curIdMain = new();
        private Guid _tempNewIdMain = new();
        private BindingList<TrinhDoHocVanDTO> _bindingListMain = new();

        #region Contructor & FormLoad
        public FrmDMTrinhDoHocVan() {
            InitializeComponent();
            IBusObj = new DMTrinhDoHocVanClient(_httpClient);
        }
        ...
        private async Task LoadDataMainGrid() {
            try {
                ...
                _bindingListMain = new BindingList<TrinhDoHocVanDTO>(res.Result.OrderBy(x => x.MaSo).ToList());
        ...
        private async void btnSubmit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            try {
                ...
                // Validate
                clsCommon.CommonHandler.ClearEditControlErrorText(layoutControlGroupEditControl);
                var validator = new TrinhDoHocVanValidator();
                ...
                if (_mainStatus == MainStatusForm.CREATE) {
                    List<TrinhDoHocVanDTO> data = new List<TrinhDoHocVanDTO> { obj };
                    ...
                    var objToEdit = (TrinhDoHocVanDTO)gridViewMain.GetRow(gridViewMain.FocusedRowHandle);
        ...
        private async void btnCancel_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            if (gridViewMain.RowCount > 0) {
                var objMain = (TrinhDoHocVanDTO)gridViewMain.GetFocusedRow();
