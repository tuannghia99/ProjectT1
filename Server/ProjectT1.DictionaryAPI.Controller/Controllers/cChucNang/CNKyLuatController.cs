...
namespace ProjectT1.DictionaryAPI.Controller {
    [ApiController]
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/chucNang/KyLuat")]
    public class CNKyLuatController(IKyLuatService service) : ControllerBase() {
        [HttpGet]
        public async Task<ActionResult<OperationResultInfo<IEnumerable<KyLuatDTO>>>> GetAll() {
        ...
        [HttpGet]
        [Route("{Id}")]
        public async Task<ActionResult<OperationResultInfo<KyLuatDTO>>> GetById(Guid Id) {
            var (Result, Code, Message) = await service.GetById(Id);
            return StatusCode(Code, clsCommon.ApiResponse(Result, Code, Message));
        }

        [HttpPost]
        public async Task<ActionResult<OperationResultInfo<IEnumerable<KyLuatDTO>>>> Post(IEnumerable<KyLuatDTO> dataSource) {
        ...
        [HttpPut]
        [Route("{Id}")]
        public async Task<ActionResult<OperationResultInfo<KyLuatDTO>>> Put(KyLuatDTO objSource, Guid Id) {
