...
namespace ProjectT1.DictionaryAPI.Controller {
    [ApiController]
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/chucNang/NhanVien")]
    public class CNNhanVienController(INhanVienService service) : ControllerBase() {
        [HttpGet]
        public async Task<ActionResult<OperationResultInfo<IEnumerable<NhanVienDTO>>>> GetAll() {
        ...
        [HttpGet]
        [Route("{Id}")]
        public async Task<ActionResult<OperationResultInfo<NhanVienDTO>>> GetById(Guid Id) {
            var (Result, Code, Message) = await service.GetById(Id);
            return StatusCode(Code, clsCommon.ApiResponse(Result, Code, Message));
        }

        [HttpPost]
        public async Task<ActionResult<OperationResultInfo<IEnumerable<NhanVienDTO>>>> Post(IEnumerable<NhanVienDTO> dataSource) {
        ...
        [HttpPut]
        [Route("{Id}")]
        public async Task<ActionResult<OperationResultInfo<NhanVienDTO>>> Put(NhanVienDTO objSource, Guid Id) {
