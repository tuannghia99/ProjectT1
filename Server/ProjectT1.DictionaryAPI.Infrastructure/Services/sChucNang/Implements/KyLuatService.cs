...
namespace ProjectT1.DictionaryAPI.Infrastructure.Services {
    public class KyLuatService(
        ...
        IMapper _mapper,
        ILogger<KyLuatService> _logger,
        IValidator<KyLuatDTO> _validator
        ) : IKyLuatService {

        public async Task<(IEnumerable<KyLuatDTO> Result, int Code, string Message)> GetAll() {
            _logger.LogInformation("GetAll called");
            try {
                var data = await _context.KyLuats.AsNoTracking().ToListAsync();
                var res = data.Select(_mapper.Map<KyLuat, KyLuatDTO>).ToList();
        ...
        public async Task<(KyLuatDTO Result, int Code, string Message)> GetById(Guid id) {
            _logger.LogInformation("GetById called: Id {Id}", id);
            try {
                var obj = await _context.KyLuats.AsNoTracking().FirstOrDefaultAsync(x => x.Oid == id);
                if (obj == null)
                    return (null, StatusCodes.Status404NotFound, null);

                var res = _mapper.Map<KyLuat, KyLuatDTO>(obj);
        ...
        public async Task<(IEnumerable<KyLuatDTO> Result, int Code, string Message)> Post(IEnumerable<KyLuatDTO> dataSource) {
            _logger.LogInformation("Post called: DataSource {DataSource}", JsonConvert.SerializeObject(dataSource));
            using var transaction = await _context.Database.BeginTransactionAsync();
            try {
                var lstMsCode = await _context.KyLuats.AsNoTracking().Select(x => x.MaSo).ToListAsync();
                ...
                var dataDest = dataSource.Select(_mapper.Map<KyLuatDTO, KyLuat>).ToList();

                await _context.KyLuats.AddRangeAsync(dataDest);
        ...
        public async Task<(KyLuatDTO Result, int Code, string Message)> Put(KyLuatDTO objSource, Guid id) {
            _logger.LogInformation("Put called: ObjSource {ObjSource}, Id {id}", JsonConvert.SerializeObject(objSource), id);
            using var transaction = await _context.Database.BeginTransactionAsync();
            try {
                var objDest = await _context.KyLuats.FirstOrDefaultAsync(x => x.Oid == id);
                if (objDest == null)
                    return (null, StatusCodes.Status404NotFound, null);
        ...
        public async Task<(Guid? Result, int Code, string Message)> Delete(Guid id) {
            ...
            try {
                ...
                _context.KyLuats.Remove(objDest);
                await _context.SaveChangesAsync();
                await transaction.CommitAsync();
