...
namespace ProjectT1.DictionaryAPI.Infrastructure.Services {
    public class TrinhDoHocVanService(
        ...
        IMapper _mapper,
        ILogger<TrinhDoHocVanService> _logger,
        IValidator<TrinhDoHocVanDTO> _validator
        ) : ITrinhDoHocVanService {
        const string _category = "TrinhDoHocVan";

        public async Task<(IEnumerable<TrinhDoHocVanDTO> Result, int Code, string Message)> GetAll() {
            _logger.LogInformation("GetAll called");
            try {
                var data = await _context.CommonCategories.AsNoTracking().Where(x => x.CategoryId == _category).ToListAsync();
                var res = data.Select(_mapper.Map<CommonCategory, TrinhDoHocVanDTO>).ToList();
        ...
        public async Task<(TrinhDoHocVanDTO Result, int Code, string Message)> GetById(Guid id) {
            ...
            try {
                ...
                var res = _mapper.Map<CommonCategory, TrinhDoHocVanDTO>(obj);
        ...
        public async Task<(IEnumerable<TrinhDoHocVanDTO> Result, int Code, string Message)> Post(IEnumerable<TrinhDoHocVanDTO> dataSource) {
            ...
            try {
                ...
                var dataDest = dataSource.Select(_mapper.Map<TrinhDoHocVanDTO, CommonCategory>).ToList();
                dataDest.ForEach(x => {
        ...
        public async Task<(TrinhDoHocVanDTO Result, int Code, string Message)> Put(TrinhDoHocVanDTO objSource, Guid id) {
